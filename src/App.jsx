...
const SysMsg = ({msg,color,onDone}) => {
  ...
  return (
    <div style={{position:"fixed",top:60,left:"50%",transform:"translateX(-50%)",zIndex:600,maxWidth:340,width:"90%",animation:"fadeIn .2s ease"}}>
      <div style={{background:"rgba(7,7,16,.97)",border:`1px solid ${color||"#2979ff"}`,borderRadius:8,padding:"10px 14px",boxShadow:`0 0 20px ${color||"#2979ff"}44`}}>
        <div style={{fontSize:8,color:color||"#2979ff",fontFamily:"'Orbitron',monospace",letterSpacing:2,marginBottom:4}}>[ SYSTEM ]</div>
        <div style={{fontSize:12,color:"#ccc",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.5}}>{msg}</div>
      </div>
    </div>
  );
};
      ...
      <div className="slide" onClick={e=>e.stopPropagation()} style={{background:"#0d0d1a",border:`1px solid ${isHabit?"#ffd600":cfg.color}44`,borderRadius:radius,padding:"22px 20px 34px",width:"100%",maxWidth:mw,boxShadow:`0 -10px 40px ${isHabit?"rgba(255,214,0,.3)":cfg.glow}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div>
            ...
            <div style={{fontSize:10,color:isHabit?"#ffd600":cfg.color,marginTop:2}}>{cfg.icon} {cfg.name.toUpperCase()} · {isHabit?"Repeatable anytime":"Daily reset"}</div>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:"none",color:"#444",fontSize:18,cursor:"pointer"}}>✕</button>
        </div>
        <div style={{marginBottom:14}}>
          <div style={{fontSize:9,color:"#444",fontFamily:"'Orbitron',monospace",letterSpacing:1,marginBottom:6}}>{isHabit?"HABIT NAME":"TASK NAME"}</div>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder={isHabit?"e.g. 10 pull-ups":"e.g. Run 5km"} autoFocus
            ...
            style={{width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:6,color:"#fff",padding:"10px 12px",fontFamily:"'Rajdhani',sans-serif",fontSize:15,outline:"none"}}/>
        </div>
        {isHabit&&<div style={{background:"rgba(255,214,0,.06)",border:"1px solid rgba(255,214,0,.15)",borderRadius:7,padding:"9px 11px",marginBottom:14}}>
          <div style={{fontSize:10,color:"#ffd600",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.5}}>💡 Habits can be logged multiple times per day. Each press gives XP and is recorded separately.</div>
        </div>}
        <div style={{marginBottom:20}}>
          <div style={{fontSize:9,color:"#444",fontFamily:"'Orbitron',monospace",letterSpacing:1,marginBottom:8}}>XP PER LOG</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
            {[["E","10 XP","Easy"],["B","25 XP","Moderate"],["S","50 XP","Hard"]].map(([r,xp,label])=>(
              <button key={r} onClick={()=>setRank(r)} style={{background:rank===r?`${isHabit?"#ffd600":cfg.color}18`:"rgba(255,255,255,.02)",border:`1px solid ${rank===r?(isHabit?"#ffd600":cfg.color):"rgba(255,255,255,.07)"}`,borderRadius:7,padding:"10px 6px",cursor:"pointer",transition:"all .15s"}}>
                <div style={{fontFamily:"'Orbitron',monospace",fontSize:16,fontWeight:700,color:rank===r?(isHabit?"#ffd600":cfg.color):"#444"}}>{r}</div>
                <div style={{fontSize:11,color:rank===r?"#fff":"#333",fontWeight:600,marginTop:2}}>{xp}</div>
                <div style={{fontSize:9,color:"#333",marginTop:1}}>{label}</div>
              </button>
            ))}
          </div>
        </div>
        <button className="pb" onClick={()=>{if(name.trim()) onAdd(name.trim(),rank);}} style={{width:"100%",background:`linear-gradient(135deg,${isHabit?"#ffd60099,#ffd600":`${cfg.color}99,${cfg.color}`})`,color:"#000",border:"none",borderRadius:8,padding:"13px",cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:700,letterSpacing:1.5,opacity:name.trim()?1:.35}}>
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.84)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div className="slide" onClick={e=>e.stopPropagation()} style={{background:"#0d0d1a",border:"1px solid rgba(255,255,255,.15)",borderRadius:12,padding:"22px 20px",width:"100%",maxWidth:380}}>
        ...
        <input value={val} onChange={e=>setVal(e.target.value)} autoFocus
          ...
          style={{width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.15)",borderRadius:6,color:"#fff",padding:"11px 12px",fontFamily:"'Rajdhani',sans-serif",fontSize:15,outline:"none",marginBottom:14}}/>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",color:"#777",borderRadius:7,padding:"10px",cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:10}}>CANCEL</button>
          <button onClick={()=>{if(val.trim()) onSave(val.trim());}} style={{flex:1,background:"linear-gradient(135deg,#2979ff99,#2979ff)",color:"#fff",border:"none",borderRadius:7,padding:"10px",cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:700}}>SAVE</button>
        </div>
      </div>
    </div>
  );
  ...
  <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
    <div className="slide" onClick={e=>e.stopPropagation()} style={{background:"#0d0d1a",border:"1px solid rgba(255,68,68,.45)",borderRadius:12,padding:"28px 22px",width:"100%",maxWidth:380,boxShadow:"0 0 40px rgba(255,68,68,.2)"}}>
      <div style={{textAlign:"center",marginBottom:22}}>
        ...
        <div style={{fontSize:12,color:"#555",lineHeight:1.7}}>Wipes all XP, levels, history, and logs. Your name is kept. Cannot be undone.</div>
      </div>
      <div style={{display:"flex",gap:10}}>
        ...
        <button onClick={onConfirm} style={{flex:1,background:"linear-gradient(135deg,#ff444499,#ff4444)",color:"#fff",border:"none",borderRadius:7,padding:"11px",cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:700}}>CONFIRM RESET</button>
      </div>
    </div>
  </div>
);
    ...
    <div className="fade" style={{padding:isDesktop?"28px 32px 0":"20px 16px 0"}}>
      ...
      <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:8,marginBottom:14,scrollbarWidth:"none"}}>
        {["all",...Object.keys(STAT_CONFIG).filter(k=>k!=="will")].map(k=>{
          ...
          return <button key={k} onClick={()=>setFilter(k)} style={{background:on?c:"rgba(255,255,255,.04)",color:on?"#000":"#444",border:`1px solid ${on?c:"rgba(255,255,255,.07)"}`,borderRadius:20,padding:"3px 11px",cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:8,fontWeight:700,letterSpacing:.5,whiteSpace:"nowrap",flexShrink:0,transition:"all .15s"}}>{k==="all"?"ALL":STAT_CONFIG[k]?.icon+" "+k.slice(0,3).toUpperCase()}</button>;
        })}
      </div>
      {allDates.length===0&&<div style={{textAlign:"center",color:"#2a2a3a",fontSize:12,fontFamily:"'Share Tech Mono',monospace",marginTop:60,lineHeight:2}}>No activity yet.<br/>Complete tasks to build your log.</div>}
      {allDates.map(date=>{
        ...
        return (
          <div key={date} style={{marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
              <div style={{fontFamily:"'Orbitron',monospace",fontSize:9,color:"#2979ff",letterSpacing:1}}>{new Date(date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</div>
              {totalXP>0&&<div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:9,color:"#ffd600"}}>+{totalXP}</div>}
            </div>
            <div style={{background:"#0d0d1a",border:"1px solid rgba(255,255,255,.06)",borderRadius:8,overflow:"hidden"}}>
              {dayTasks.map((t,i)=>{
                const cfg=STAT_CONFIG[t.stat];
                return (
                  <div key={i} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 12px",borderBottom:i<dayTasks.length-1||daySleep?"1px solid rgba(255,255,255,.04)":"none"}}>
                    <span style={{fontSize:12}}>{cfg?.icon}</span>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,color:"#bbb",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.taskName||"Task"}</div>
                      <div style={{fontSize:8,color:"#333",fontFamily:"'Orbitron',monospace",marginTop:1}}>{cfg?.name?.toUpperCase()} · {t.rank}-RANK{t.isHabit?" · HABIT":t.isCustom?" · CUSTOM":""}</div>
                    </div>
                    <div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:cfg?.color,flexShrink:0}}>+{t.xp}</div>
                  </div>
                );
              })}
              {daySleep&&(filter==="all"||filter==="health")&&(
                <div style={{display:"flex",alignItems:"center",gap:9,padding:"9px 12px"}}>
                  ...
                  <div style={{flex:1}}><div style={{fontSize:12,color:"#bbb",fontWeight:600}}>{daySleep.hours}h sleep</div><div style={{fontSize:8,color:"#333",fontFamily:"'Orbitron',monospace",marginTop:1}}>HEALTH · SLEEP</div></div>
                  <div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:"#ff4081",flexShrink:0}}>+{daySleep.xp}</div>
                </div>
              )}
            </div>
          </div>
...
export default function App() {
  ...
  const logSleep = () => {
    ...
    const xp=h>=8?30:h>=7?20:h>=6?10:0,today=todayStr();
    setState(prev=>{
  ...
  if(!state) return (
    <div style={{background:"#000",height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12}}>
      <div style={{fontFamily:"'Orbitron',monospace",color:"#2979ff",fontSize:13,letterSpacing:3}}>INITIALIZING</div>
      <div style={{display:"flex",gap:5}}>{[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#2979ff",animation:`pulse ${.8+i*.2}s ${i*.2}s infinite`}}/>)}</div>
    </div>
  );
